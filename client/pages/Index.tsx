...
import {
  computeItem,
  getUnitLabel,
  DEFAULT_RATES as EST_DEFAULT_RATES,
  findItemDef,
...
const CONSTRUCTION_ITEMS = {
  foundation: {
    ...
    items: [
      ...
      { id: "lean_concrete", name: "Lean Concrete (PCC)", unit: "cft" },
...
export default function Index() {
  ...
  const handleAddItem = useCallback(() => {
    ...
    if (def?.mode === "wall") {
      ...
      valid =
        has(formData.length) &&
        has(formData.width) &&
        (has(formData.height) || has(formData.thickness || ""));
    }
    if (!valid) {
      toast({
        title: "Missing Information",
        description: "Dimensions incomplete for selected item.",
        variant: "destructive",
      });
      return;
    }
    ...
    const itemInfo =
      categoryInfo?.items.find((item) => item.id === formData.type) ||
      ({ id: "custom", name: "Custom Item", unit: "unit" } as any);
    ...
    const newItem: EstimateItem = {
      ...
      description:
        ...
        `${itemInfo?.name} - ${formData.length}' × ${formData.width}' × ${formData.height}'`,
      dimensions: {
  ...
  const projectSummary: ProjectSummary = {
    ...
    totalCost: items.reduce(
      (sum, item) => sum + (item.subtotal ?? item.totalCost),
      0,
    ),
    categoryBreakdown: items.reduce(
      (acc, item) => {
  ...
  const totals = computeProjectTotals(
    items.map((i) => i.subtotal ?? i.totalCost),
    customRates as any,
  );
  ...
  const mobileTableItems = filteredItems.map((item) => {
    const categoryInfo = Object.values(CONSTRUCTION_ITEMS).find(
    ...
    return {
      ...
      plasterArea:
        item.type.includes("plaster") ||
        item.type.includes("paint") ||
        item.type.includes("tiles")
          ...
          {activeTab === "summary" && (
            <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="text-center p-3 bg-green-50 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-green-600">
                        {Math.round(projectSummary.totalVolume)}
                      </p>
                    ...
                    <div className="text-center p-3 bg-orange-50 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-orange-600">
                        {Math.round(projectSummary.totalSteel)}
                      </p>
                      <p className="text-sm text-gray-600">
                        Steel Required (kg)
                      </p>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {Object.entries(projectSummary.categoryBreakdown).map(
                      ([category, data]) => {
                        const categoryInfo = Object.values(
                          CONSTRUCTION_ITEMS,
                        ).find((cat) => cat.label === category);
                        const IconComponent = categoryInfo?.icon || Building2;
                        const percentage =
                          (data.cost / projectSummary.totalCost) * 100;
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center space-x-3">
                              <div
                                className={`p-2 rounded-lg ${categoryInfo?.bgColor || "bg-gray-100"}`}
                              >
                                <IconComponent
                                  className={`h-4 w-4 ${categoryInfo?.color || "text-gray-600"}`}
                                />
                              </div>
                              <div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        {Object.entries(CONSTRUCTION_ITEMS).map(
                          ([key, category]) => (
                            <div key={key}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="length">
                        Length ({getUnitLabel(formData.type, "length")}) *
                      </Label>
                      <Input
                        id="length"
                        type="number"
                    ...
                    <div>
                      <Label htmlFor="width">
                        Width ({getUnitLabel(formData.type, "width")}) *
                      </Label>
                      <Input
                        id="width"
                        type="number"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="height">
                        Height ({getUnitLabel(formData.type, "height")}) *
                      </Label>
                      <Input
                        id="height"
                        type="number"
                    ...
                    <div>
                      <Label htmlFor="thickness">
                        Thickness ({getUnitLabel(formData.type, "thickness")})
                      </Label>
                      <Input
                        id="thickness"
                        type="number"
                        ...
                        placeholder="0.33"
                        disabled={
                          getUnitLabel(formData.type, "thickness") === "-"
                        }
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setFormData((prev) => ({
                            ...prev,
                            multipleQuantity: e.target.value,
                          }))
                        }
                      />
                    </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                        aggregate: parseFloat(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                        steel: parseFloat(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    id="wastage"
                    type="number"
                    value={customRates.wastagePercent}
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                ...
                <div>
                  ...
                  <Input
                    id="overhead"
                    type="number"
                    value={customRates.overheadPercent}
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                ...
                <div>
                  ...
                  <Input
                    id="profit"
                    type="number"
                    value={customRates.profitPercent}
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                ...
                <div>
                  ...
                  <Input
                    ...
                    type="number"
                    step="0.01"
                    value={customRates.dryFactor}
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                        dryFactor: parseFloat(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    type="number"
                    step="0.01"
                    value={customRates.cementBagVolumeCft}
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                        cementBagVolumeCft: parseFloat(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <Input
                      type="number"
                      placeholder="1"
                      value={customRates.concreteMix.c}
                      onChange={(e) =>
                        setCustomRates((p: any) => ({
                          ...p,
                          concreteMix: {
                    ...
                    <Input
                      type="number"
                      placeholder="1.5"
                      value={customRates.concreteMix.s}
                      onChange={(e) =>
                        setCustomRates((p: any) => ({
                          ...p,
                          concreteMix: {
                    ...
                    <Input
                      type="number"
                      placeholder="3"
                      value={customRates.concreteMix.a}
                      onChange={(e) =>
                        setCustomRates((p: any) => ({
                          ...p,
                          concreteMix: {
                ...
                <div>
                  ...
                  <Input
                    ...
                    type="number"
                    step="0.01"
                    value={customRates.steelFactor}
                    onChange={(e) =>
                      setCustomRates((p: any) => ({
                        ...p,
                        steelFactor: parseFloat(e.target.value) || 0,
                      }))
                    }
                  />
                </div>
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="flex items-center justify-center w-16 h-16 bg-white rounded-xl ring-1 ring-black/5">
                <img
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600">
                {projectSummary.totalItems} items •{" "}
                {formatBDT(totals.grandTotal)} total
              </p>
            </div>
            <div className="flex items-center space-x-3">
                    ...
                    ([category, data]) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            <div
                              className={`p-3 rounded-lg ${categoryInfo?.bgColor || "bg-gray-100"}`}
                            >
                              <IconComponent
                                className={`h-6 w-6 ${categoryInfo?.color || "text-gray-600"}`}
                              />
                            </div>
                            <div>
                              <p className="font-medium">{category}</p>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <Select
                        value={formData.mainBars}
                        onValueChange={(value) =>
                          setFormData((prev) => ({ ...prev, mainBars: value }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                    ...
                    <div>
                      ...
                      <Select
                        value={formData.stirrups}
                        onValueChange={(value) =>
                          setFormData((prev) => ({ ...prev, stirrups: value }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
